...
class RandomForestClassifier:
    ...
    def predict_single_tree(self, predict, X, dX, out):
        ...
        print("Predict Single Tree Break Test 1")
        prediction = predict(X,dX)

        #print(prediction[:10])

        print("Predict Single Tree Break Test 2")
        if len(out) == 1:
            print("Predict Single Tree Break Test 3")
            out[0] += prediction
            #out2[0] += prediction[1]
        else:
            print("Predict Single Tree Break Test 4")
    ...
    def predict_proba(self, X, dX=None):
        ...
        """
        print("Predict Proba Break Test 1")
        ...
        print("Predict Proba Break Test 2")
        X, dX = self.check_input_X(X, dX)

        print("Predict Proba Break Test 3")
        for i, tree in enumerate(self.estimators_):
            tree.node_arr_init()
            print("Predict Proba Break Test 4")
            self.predict_single_tree(tree.predict_proba, X, dX, proba)
            print("Predict Proba Break Test 5")

        proba /= self.n_estimators_
        print("Predict Proba Break Test 6")
    ...
    def predict(self, X, dX=None, return_leafs=False):
        print("Predict Break Test 1")
        print(self.predict_proba(X, dX))
        y_pred_inds = numpy.argmax(self.predict_proba(X, dX), axis = 1)
        print("Predict Break Test 2")
        print([self.label_dict[i] for i in y_pred_inds])
        y_pred = numpy.array([self.label_dict[i] for i in y_pred_inds])
        return y_pred

    def score(self, X, y, dX=None):
        print("Score Break Test 1")
        y_pred = self.predict(X, dX)
        print("Score Break Test 2")
        score = (y_pred == (y)).sum()/len(y)
        print("Score Break Test 3")
        return score
