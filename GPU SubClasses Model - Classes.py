...
subclasses = ['clasCep','t2Cep','contactBinary','detachedBinary','dsct','Mira','OSARG','SRV','RRab','RRc']
...
def replicate(files):
    ...
    for file, num in files:
        for singleclass in classes:
            if singleclass in file:
                aux_dic[singleclass].append([file, num])
                break

    new_files = []
    for singleclass in classes:
        array = aux_dic[singleclass]
        length = len(array)
        if length == 0:
            continue
...
def experiment(directory, files, Y, classes, N, n_splits):
    ...
    for early in earlyStopping:
        for activation in activations:
            ...
            text_file = open("ResultsClasses/Model Accuracy.txt", "a")
...
YSingleClass = []
for file, num in files:
    YSingleClass.append(get_name_with_survey(file))
YSingleClass = np.array(YSingleClass)
...
while NUMBER_OF_POINTS <= MAX_NUMBER_OF_POINTS:
    ...
    experiment(directory, files, YSingleClass, classes, NUMBER_OF_POINTS, n_splits)
